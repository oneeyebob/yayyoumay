...
export default function PinSetupPage() {
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    if (!entry.complete || !confirm.complete) {
      setError('Please fill in both PINs.')
      return
    }
  ...
  return (
    <main className="min-h-screen flex items-center justify-center bg-gray-50 px-4">
      <div className="w-full max-w-sm">
        <h1 className="text-2xl font-bold text-gray-900 mb-2">Set your curator PIN</h1>
        <p className="text-sm text-gray-500 mb-8">
          You&apos;ll use this 4-digit PIN to access the curator area from the profile picker.
        </p>

        <form onSubmit={handleSubmit} className="space-y-8">
          {/* PIN entry */}
          <div>
            <p className="text-sm font-medium text-gray-700 mb-3">Choose a PIN</p>
          ...
          {/* PIN confirm */}
          <div>
            <p className="text-sm font-medium text-gray-700 mb-3">Confirm PIN</p>
            <div className="flex gap-3">
